...
import static org.mongkie.mimi.MiMI.AnnotationElement.*;
import org.mongkie.mimi.MiMI.PPI;
...
public class MiMITest {
    ...
    public void testQuery() throws JAXBException, MalformedURLException {
        ...
        System.out.println();
        Integer[] geneIds = new Integer[]{1436, 8563, 50511};
        ...
        for (Integer sourceGeneId : results.keySet()) {
            ...
            for (PPI i : results.get(sourceGeneId)) {
                for (Attribute a : i.getInteractor().getAttributes()) {
                    Object v = a.getValue();
                    System.out.println("\t" + a.getName() + ": " + (v instanceof String[] ? Arrays.toString((String[]) v) : v));
                }
    ...
    @Test
    public void testAnnotate() throws JAXBException, MalformedURLException {
