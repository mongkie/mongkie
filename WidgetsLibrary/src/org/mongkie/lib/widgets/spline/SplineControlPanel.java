 ...
 */
package org.mongkie.lib.widgets.spline;
        ...
        public void valueChanged(ListSelectionEvent e) {
            ...
            if (template != null) {
                if (controller != null && controller.isRunning()) {
                    controller.getTimingSource().dispose();
                    controller.stop();
                }

                controller = new AnimatorBuilder(new SwingTimerTimingSource()).setDuration(300, TimeUnit.MILLISECONDS).addTarget(
                        PropertySetter.getTarget(display, "control1",
                        new KeyFramesBuilder<Point2D>().setEvaluator(point2dInterpolator).addFrames(display.getControl1(),
                        template.getControl1()).build())).build();
//                controller.setResolution(10);
                controller.addTarget(PropertySetter.getTarget(display, "control2",
                        new KeyFramesBuilder<Point2D>().setEvaluator(point2dInterpolator).addFrames(display.getControl2(),
                        template.getControl2()).build()));
                controller.getTimingSource().init();
                controller.start();
            }
        }
    }
