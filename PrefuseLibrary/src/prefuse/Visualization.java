...
/**
 * <p>Central data structure representing an interactive Visualization. This
 * class is responsible for managing the mappings between source data and
 * onscreen VisualItems, maintaining a list of {@link Display} instances
 * responsible for rendering of and interaction with the contents of this
 * visualization, and providing a collection of named Action instances for
 * performing data processing such as layout, animation, and size, shape, and
 * color assignment.</p>
 *
 * <p>The primary responsibility of the Visualization class is the creation of
 * <em>visual abstractions</em> of input data. Regardless of the data structure
 * (i.e., {@link prefuse.data.Table}, {@link prefuse.data.Graph}, or {@link prefuse.data.Tree}),
 * this class takes source data such as that loaded from a file (see {@link prefuse.data.io})
 * or from a relational database (see {@link prefuse.data.io.sql}) and creates a
 * visual representation of the data. These visual representations of the data
 * are data sets in their own right, providing access to the underlying source
 * data to be visualized while also adding addition data fields specific to a
 * visualization. These fields include spatial location (x, y coordinates and
 * item bounds), color (for stroke, fill, and text), size, shape, and font. For
 * a given input data set of type
 * {@link prefuse.data.Table}, {@link prefuse.data.Graph}, or or {@link prefuse.data.Tree},
 ...
 * visual variables unique to that generated visual abstraction. Similarly, all {@link prefuse.data.Tuple},
 * {@link prefuse.data.Node}, or {@link prefuse.data.Edge} instances used to
 * represent an entry in the source data have a corresponding {@link prefuse.visual.VisualItem},
 ...
 *
 * <p>The mapping of source data to a visual abstraction is accomplished using {@link #add(String, TupleSet)}
 * and the other "add" methods. These methods will automatically create the
 * visual abstraction, and store it in this visualization, associating it with a
 * provided <em>data group name </em>. This group name allows for queries to
 * this visualization that consider only VisualItem instances from that
 * particular group. This is quite useful when crafting {@link prefuse.action.Action}
 * instances that process only a particular group of visual data. The
 * Visualization class provides mechanisms for querying any or all groups within
 * the visualization, using one or both of the group name or a filtering
 * {@link prefuse.data.expression.Predicate} to determine the items to include
 * (see {@link #items(Predicate)} for an examples). Source data may be added
 * multiple times to a Visualization under different group names, allowing for
 * multiple representations of the same backing data.</p>
 *
 * <p>Additionally, the Visualization class supports VisualItem instances that
 * are not directly grounded in backing source data. Examples include
 ...
 * groups of these kinds include {@link #addDecorators(String, String)} and {@link #addAggregates(String)}.</p>
 *
 ...
 * provided. The Visualization class includes some default group names, namely {@link #FOCUS_ITEMS},
 * {@link #SELECTED_ITEMS}, and {@link #SEARCH_ITEMS} for the above mentioned
 * tasks. By default, both the {@link #FOCUS_ITEMS},
 ...
 * some of the interactive controls provided by the
 ...
 * instances providing interactive views of the content of this visualization. {@link Display}
 * instances registers themselves with the visualization either in their
 * constructor or through the {@link Display#setVisualization(Visualization)}
 * method, so they do not otherwise need to be added manually. Displays can be
 * configured to show all or only a subset of the data in the Visualization. A
 * filtering {@link prefuse.data.expression.Predicate} can be used to control
 * what items are drawn by the displaying, including limiting the Display to
 * particular data groups (for example, using a
 ...
 * processing on the VisualItems contained in the visualization. Using the {@link #putAction(String, Action)}
 * will add a named Action to the visualization, registering the Action such
 * that a reference to this Visualization will be available within the scope of
 * the Action's {@link prefuse.action.Action#run(double)} method. Processing
 * Actions can later be invoked by name using the {@link #run(String)} method
 * and other similar methods. This functionality not only provides a convenient
 * means of organizing a Visualization-specific collection of processing
 * Actions, it also allows for a layer of indirection between an Action and its
 * name. This allows Actions to be dynamically swapped at runtime. For example,
 * an application may make a call to invoke an Action named "layout", but the
 * actual layout processing maybe be dynamically swapped by changing the Action
 * that corresponds to that name. For more information on processing Actions,
 * see the
...
public class Visualization {
    ...
    public static final String SELECTED_TARGETS = PrefuseConfig.get("visualization.selectedTargets");
    /**
    ...
    public static final String AGGR_ITEMS = "_aggr_";
    public static final String DRAW = "_draw_";
    public static final String ANIMATE = "_animate_";
    public static final String LAYOUT = "_layout_";
    // visual abstraction
    ...
    public Visualization() {
        ...
        addFocusGroup(SEARCH_ITEMS, new DefaultTupleSet());
        addFocusGroup(SELECTED_TARGETS, new DefaultTupleSet());
...
//        return addAggregates(group, VisualItem.SCHEMA);
        return addAggregates(group, VisualItem.AGGREGATE_SCHEMA);
    }
    ...
    public synchronized void reset() {
        ...
        for (Iterator<VisualTupleSet> visualGroupIter = m_visual.values().iterator(); visualGroupIter.hasNext();) {
            VisualTupleSet visualGroup = visualGroupIter.next();
            visualGroup.removeAllTupleSetListener();
            if (visualGroup instanceof VisualTable) {
                ((VisualTable) visualGroup).removeAllTableListeners();
            }
        }
        m_visual.clear();
        m_source.clear();
    ...
    private AggregateTable initAggregates() {
        AggregateTable aggrTable = addAggregates(AGGR_ITEMS);
        aggrTable.addColumn(VisualItem.POLYGON, float[].class);
        aggrTable.addColumn(AggregateItem.AGGR_ID, int.class);
        aggrTable.addColumn(AggregateItem.AGGR_NAME, String.class);
        return aggrTable;
    }
     ...
     * visualization reference held by the removed Action will be set to
     * null.<br/> <strong>NOTE:</strong> Errors may occur if the removed Action
     * is included in an "always run after" relation with another registered
    ...
    public synchronized void rerun(Runnable closure) {
    ...
    public synchronized void rerun(Runnable closure, String... activities) {
