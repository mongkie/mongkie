 ...
 * renderers.</p>
 *
 * <p> This class supports the use of a default renderer for EdgeItems (the
 * default edge renderer) and another for all other non-edge VisualItems (the
 * default item renderer). In addition, any number of additional Renderer
 * mapping rules can be added, by specifying a Predicate to apply and a Renderer
 * to return for matching items. Predicate/Renderer mappings are checked in the
 * order in which they were added to the factory. </p>
 *
 * <p>If left unspecified, a {@link ShapeRenderer} is used as the default item
 ...
 * EdgeRenderer using curved edges, and adds a new rule which maps items in the
 * group "data" to a text renderer that pulls its text from a field named
 * "label".</p>
 * <pre>
...
public class DefaultRendererFactory implements RendererFactory {
    ...
    public DefaultRendererFactory(Renderer itemRenderer, Renderer edgeRenderer) {
        this(itemRenderer, edgeRenderer, null);
    }

    public DefaultRendererFactory(Renderer itemRenderer, Renderer edgeRenderer, Renderer aggrRenderer) {
     ...
     * renderer for non-EdgeItems, see
    ...
    public void setAggregateRenderer(Visualization v, Renderer r) {
        m_aggrRenderer = r;
        if (r != null) {
            v.invalidate(Visualization.AGGR_ITEMS);
        }
    }
     ...
     * the Renderer for the earliest match will be returned.
     *
     * @param predicate a String in the prefuse expression language. This String
     * will be parsed to create a corresponding Predicate instance.
     * @param r the Renderer to return if an item matches the Predicate
     */
    ...
    public Renderer getRenderer(VisualItem item) {
        ...
        if (r != null) {
            ...
            return m_aggrRenderer != null ? m_aggrRenderer : NULL_RENDERER;
