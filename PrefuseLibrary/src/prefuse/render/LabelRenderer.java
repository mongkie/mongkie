...
public class LabelRenderer extends AbstractShapeRenderer {
    ...
    public void render(Graphics2D g, VisualItem item) {
        ...
        if ( text != null && ColorLib.alpha(textColor) > 0 ) {
            ...
            for ( ; end >= 0; y += lh ) {
                drawString(item, g, fm, text.substring(start, end), useInt, x, y, tw);
                start = end+1;
                end = text.indexOf(m_delim, start);   
            }
            drawString(item, g, fm, text.substring(start), useInt, x, y, tw);
        }
    ...
    protected void drawString(VisualItem item, Graphics2D g, FontMetrics fm, String text,
            boolean useInt, double x, double y, double w) {
        drawString(g, fm, text, useInt, x, y, w);
    }
