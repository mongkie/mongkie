 ...
 * testing methods of the {@link Shape} interface. <p> Use {@link Path2D.Float}
 * when dealing with data that can be represented and used with floating point
 * precision. Use {@link Path2D.Double} for data that requires the accuracy or
 * range of double precision. <p> {@code Path2D} provides exactly those
 * facilities required for basic construction and management of a geometric path
 * and implementation of the above interfaces with little added interpretation.
 * If it is useful to manipulate the interiors of closed geometric shapes beyond
 * simple hit testing then the {@link Area} class provides additional
 * capabilities specifically targeted at closed figures. While both classes
 * nominally implement the {@code Shape} interface, they differ in purpose and
 * together they provide two useful views of a geometric shape where
 * {@code Path2D} deals primarily with a trajectory formed by path segments and
 * {@code Area} deals more with interpretation and manipulation of enclosed
 * regions of 2D geometric space. <p> The {@link PathIterator} interface has
 * more detailed descriptions of the types of segments that make up a path and
 * the winding rules that control how to determine which regions are inside or
 * outside the path.
 *
 * @author Jim Graham
 * @since 1.6
 */
public abstract class TransientPath2D implements Shape, Cloneable {
     ...
     * @param initialTypes the size to make the initial array to store the path
     * segment types
     * @since 1.6
     */
    /* private protected */
    TransientPath2D(int rule, int initialTypes) {
        setWindingRule(rule);
     ...
     * @since 1.6
     */
    public static class Float extends TransientPath2D implements Serializable {
        ...
        public Float(Shape s, AffineTransform at) {
            if (s instanceof TransientPath2D) {
                TransientPath2D p2d = (TransientPath2D) s;
         ...
         * specified in float precision. <p> This method provides a single
         * precision variant of the double precision {@code moveTo()} method on
         * the base {@code Path2D} class.
         *
         ...
         * precision. <p> This method provides a single precision variant of the
         * double precision {@code lineTo()} method on the base {@code Path2D}
         * class.
         *
         * @param x the specified X coordinate
         * @param y the specified Y coordinate
         ...
         * point. All coordinates are specified in float precision. <p> This
         * method provides a single precision variant of the double precision
         ...
         * control points. All coordinates are specified in float precision. <p>
        ...
        /**
         * {@inheritDoc} <p> The iterator for this class is not multi-threaded
         * safe, which means that the {@code Path2D} class does not guarantee
         * that modifications to the geometry of this {@code Path2D} object do
         * not affect any iterations of that geometry that are already in
         * process.
         *
         * @since 1.6
         */
        ...
        public final Object clone() {
            return new TransientPath2D.Float(this);
        }
         ...
         *
         * @serialData <a name="Path2DSerialData"><!-- --></a> <ol> <li>The
         * default serializable fields. There are no default serializable fields
         * as of 1.6. <li>followed by a byte indicating the storage type of the
         * original object as a hint (SERIAL_STORAGE_FLT_ARRAY) <li>followed by
         * an integer indicating the number of path segments to follow (NP) or
         * -1 to indicate an unknown number of path segments follows
         * <li>followed by an integer indicating the total number of coordinates
         * to follow (NC) or -1 to indicate an unknown number of coordinates
         * follows (NC should always be even since coordinates always appear in
         * pairs representing an x,y pair) <li>followed by a byte indicating the
         * winding rule ({@link #WIND_EVEN_ODD WIND_EVEN_ODD} or
         * {@link #WIND_NON_ZERO WIND_NON_ZERO}) <li>followed by NP (or
         * unlimited if NP < 0) sets of values consisting of a single byte
         * indicating a path segment type followed by one or more pairs of float
         * or double values representing the coordinates of the path segment
         * <li>followed by a byte indicating the end of the path
         * (SERIAL_PATH_END). </ol> <p> The following byte value constants are
         * used in the serialized form of {@code Path2D} objects: <table> <tr>
         * <th>Constant Name</th> <th>Byte Value</th> <th>Followed by</th>
         * <th>Description</th> </tr> <tr>
         * <td>{@code SERIAL_STORAGE_FLT_ARRAY}</td> <td>0x30</td> <td></td>
         * <td>A hint that the original {@code Path2D} object stored the
         * coordinates in a Java array of floats.</td> </tr> <tr>
         * <td>{@code SERIAL_STORAGE_DBL_ARRAY}</td> <td>0x31</td> <td></td>
         * <td>A hint that the original {@code Path2D} object stored the
         * coordinates in a Java array of doubles.</td> </tr> <tr>
         * <td>{@code SERIAL_SEG_FLT_MOVETO}</td> <td>0x40</td> <td>2
         * floats</td> <td>A {@link #moveTo moveTo} path segment follows.</td>
         * </tr> <tr> <td>{@code SERIAL_SEG_FLT_LINETO}</td> <td>0x41</td> <td>2
         * floats</td> <td>A {@link #lineTo lineTo} path segment follows.</td>
         * </tr> <tr> <td>{@code SERIAL_SEG_FLT_QUADTO}</td> <td>0x42</td> <td>4
         * floats</td> <td>A {@link #quadTo quadTo} path segment follows.</td>
         * </tr> <tr> <td>{@code SERIAL_SEG_FLT_CUBICTO}</td> <td>0x43</td>
         * <td>6 floats</td> <td>A {@link #curveTo curveTo} path segment
         * follows.</td> </tr> <tr> <td>{@code SERIAL_SEG_DBL_MOVETO}</td>
         * <td>0x50</td> <td>2 doubles</td> <td>A {@link #moveTo moveTo} path
         * segment follows.</td> </tr> <tr>
         * <td>{@code SERIAL_SEG_DBL_LINETO}</td> <td>0x51</td> <td>2
         * doubles</td> <td>A {@link #lineTo lineTo} path segment follows.</td>
         * </tr> <tr> <td>{@code SERIAL_SEG_DBL_QUADTO}</td> <td>0x52</td> <td>4
         * doubles</td> <td>A {@link #curveTo curveTo} path segment
         * follows.</td> </tr> <tr> <td>{@code SERIAL_SEG_DBL_CUBICTO}</td>
         * <td>0x53</td> <td>6 doubles</td> <td>A {@link #curveTo curveTo} path
         * segment follows.</td> </tr> <tr> <td>{@code SERIAL_SEG_CLOSE}</td>
         * <td>0x60</td> <td></td> <td>A {@link #closePath closePath} path
         * segment.</td> </tr> <tr> <td>{@code SERIAL_PATH_END}</td>
         * <td>0x61</td> <td></td> <td>There are no more path segments
         * following.</td> </table>
         *
         * @since 1.6
         */
         ...
         * the path segments stored in this path. <p> There are no default
         * serializable fields as of 1.6. <p> The serial data for this object is
         * described in the writeObject method.
         *
         * @since 1.6
         */
        ...
        static class CopyIterator extends TransientPath2D.Iterator {

            float floatCoords[];

            CopyIterator(TransientPath2D.Float p2df) {
                super(p2df);
        ...
        static class TxIterator extends TransientPath2D.Iterator {

            float floatCoords[];
            AffineTransform affine;

            TxIterator(TransientPath2D.Float p2df, AffineTransform at) {
                super(p2df);
                this.floatCoords = p2df.floatCoords;
                this.affine = at;
            }
     ...
     * @since 1.6
     */
    public static class Double extends TransientPath2D implements Serializable {
         ...
         * of this {@code Shape} and provides access to the geometry of this
        ...
        public final Object clone() {
            ...
            return new TransientPath2D.Double(this);
        }
         ...
         * default serializable fields. There are no default serializable fields
         * as of 1.6. <li>followed by a byte indicating the storage type of the
         * original object as a hint (SERIAL_STORAGE_DBL_ARRAY) <li>followed by
         * an integer indicating the number of path segments to follow (NP) or
         * -1 to indicate an unknown number of path segments follows
         * <li>followed by an integer indicating the total number of coordinates
         * to follow (NC) or -1 to indicate an unknown number of coordinates
        ...
        static class CopyIterator extends TransientPath2D.Iterator {
            ...
            CopyIterator(TransientPath2D.Double p2dd) {
                super(p2dd);
        ...
        static class TxIterator extends TransientPath2D.Iterator {
            ...
            TxIterator(TransientPath2D.Double p2dd, AffineTransform at) {
                super(p2dd);
                this.doubleCoords = p2dd.doubleCoords;
                this.affine = at;
            }
     ...
     * {@code moveTo} segment into a {@code lineTo} segment to connect the new
     ...
     *
     * @param pi the {@code PathIterator} whose geometry is appended to this
     * path
     * @param connect a boolean to control whether or not to turn an initial
     * {@code moveTo} segment into a {@code lineTo} segment to connect the new
     * geometry to the existing path
     ...
     *
     * @param at the {@code AffineTransform} used to transform a new
     * {@code Shape}.
    ...
    public final synchronized Shape createTransformedShape(AffineTransform at) {
        TransientPath2D p2d = (TransientPath2D) clone();
        if (at != null) {
            p2d.transform(at);
        }
        return p2d;
     ...
     * specified {@link PathIterator}. <p> This method provides a basic facility
     * for implementors of the {@link Shape} interface to implement support for
     * the {@link Shape#contains(double, double)} method.
     *
     ...
     * the specified {@link PathIterator}. <p> This method provides a basic
     * facility for implementors of the {@link Shape} interface to implement
     * support for the {@link Shape#contains(Point2D)} method.
     *
     * @param pi the specified {@code PathIterator}
     ...
     * boundary of the specified {@link PathIterator}. <p> This method provides
     * a basic facility for implementors of the {@link Shape} interface to
     * implement support for the
     * {@link Shape#contains(double, double, double, double)} method. <p> This
     * method object may conservatively return false in cases where the
     ...
     * boundary of the specified {@link PathIterator}. <p> This method provides
     * a basic facility for implementors of the {@link Shape} interface to
     * implement support for the {@link Shape#contains(Rectangle2D)} method. <p>
    ...
    /**
     * {@inheritDoc} <p> This method object may conservatively return false in
     * cases where the specified rectangular area intersects a segment of the
     * path, but that segment does not represent a boundary between the interior
     * and exterior of the path. Such segments could lie entirely within the
     * interior of the path if they are part of a path with a
     * {@link #WIND_NON_ZERO} winding rule or if the segments are retraced in
     * the reverse direction such that the two sets of segments cancel each
     * other out without any exterior area falling between them. To determine
     * whether segments represent true boundaries of the interior of the path
     * would require extensive calculations involving all of the segments of the
     * path and the winding rule and are thus beyond the scope of this
     * implementation.
     *
     * @since 1.6
     */
     ...
     * the interior of a specified set of rectangular coordinates. <p> This
     * method provides a basic facility for implementors of the {@link Shape}
     * interface to implement support for the
     * {@link Shape#intersects(double, double, double, double)} method. <p> This
     * method object may conservatively return true in cases where the specified
     * rectangular area intersects a segment of the path, but that segment does
     * not represent a boundary between the interior and exterior of the path.
     * Such a case may occur if some set of segments of the path are retraced in
     * the reverse direction such that the two sets of segments cancel each
     * other out without any interior area between them. To determine whether
     * segments represent true boundaries of the interior of the path would
     * require extensive calculations involving all of the segments of the path
     * and the winding rule and are thus beyond the scope of this
     * implementation.
     *
     * @param pi the specified {@code PathIterator}
     * @param x the specified X coordinate
     ...
     * interior of the specified set of rectangular coordinates intersect each
    ...
    /**
     * Tests if the interior of the specified {@link PathIterator} intersects
     * the interior of a specified {@link Rectangle2D}. <p> This method provides
     * a basic facility for implementors of the {@link Shape} interface to
     * implement support for the {@link Shape#intersects(Rectangle2D)} method.
     * <p> This method object may conservatively return true in cases where the
     * specified rectangular area intersects a segment of the path, but that
     * segment does not represent a boundary between the interior and exterior
     ...
     * @return {@code true} if the specified {@code PathIterator} and the
     * interior of the specified {@code Rectangle2D} intersect each other;
    ...
    /**
     * {@inheritDoc} <p> This method object may conservatively return true in
     * cases where the specified rectangular area intersects a segment of the
     * path, but that segment does not represent a boundary between the interior
     * and exterior of the path. Such a case may occur if some set of segments
     * of the path are retraced in the reverse direction such that the two sets
     * of segments cancel each other out without any interior area between them.
     * To determine whether segments represent true boundaries of the interior
     * of the path would require extensive calculations involving all of the
     * segments of the path and the winding rule and are thus beyond the scope
     * of this implementation.
     *
     * @since 1.6
    ...
    /**
     * {@inheritDoc} <p> The iterator for this class is not multi-threaded safe,
     * which means that this {@code Path2D} class does not guarantee that
     * modifications to the geometry of this {@code Path2D} object do not affect
    ...
    final void writeObject(java.io.ObjectOutputStream s, boolean isdbl)
        ...
        if (isdbl) {
            dCoords = ((TransientPath2D.Double) this).doubleCoords;
            fCoords = null;
        } else {
            fCoords = ((TransientPath2D.Float) this).floatCoords;
            dCoords = null;
        }
    ...
    final void readObject(java.io.ObjectInputStream s, boolean storedbl)
        ...
        if (storedbl) {
            ((TransientPath2D.Double) this).doubleCoords = new double[nC];
        } else {
            ((TransientPath2D.Float) this).floatCoords = new float[nC];
        }
    ...
    static abstract class Iterator implements PathIterator {
        ...
        int typeIdx;
        int pointIdx;
        TransientPath2D path;
        static final int curvecoords[] = {2, 2, 4, 6, 0};

        Iterator(TransientPath2D path) {
            this.path = path;
        }
