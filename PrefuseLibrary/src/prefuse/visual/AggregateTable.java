...
public class AggregateTable extends VisualTable {
    ...
    public static final Schema AGGREGATE_SCHEMA = new Schema(
            new String[]{AggregateItem.AGGR_ID, AggregateItem.AGGR_NAME},
    ...
    public void addToAggregate(int row, VisualItem member) {
        if (addToAggregateInternal(row, member)) {
            fireTableEvent(row, row,
                    EventConstants.ALL_COLUMNS, EventConstants.UPDATE);
        }
    }

    public void addToAggregate(int row, Iterator<VisualItem> members) {
        boolean updated = false;
        while (members.hasNext()) {
            if (addToAggregateInternal(row, members.next())) {
                updated = true;
            }
        }
        if (updated) {
    ...
    public void addToAggregate(int row, Collection<? extends Tuple> tuples, String group) {
        boolean updated = false;
        for (Tuple tuple : tuples) {
            if (addToAggregateInternal(row, getVisualization().getVisualItem(group, tuple))) {
                updated = true;
            }
        }
        if (updated) {
    ...
    private boolean addToAggregateInternal(int row, VisualItem member) {
    ...
    @Override
    public void clear() {
        for (IntIterator rows = rows(); rows.hasNext();) {
            removeAllFromAggregate(rows.nextInt());
        }
        super.clear();
    }
    ...
    protected void clearAggregateMappings(int row, boolean update) {
        ...
        for (IntIterator rows = index.rows(row); rows.hasNext();) {
             ...
             * fire = true;
             */
            toRemove.add(rows.nextInt());
        }
        for (int r : toRemove) {
            ((IntIntSortedMap) index.getIntSortedMap()).remove(
                    m_aggregated.getColumn(AGGREGATE).getInt(m_aggregated.getColumnRow(r, m_aggregated.getColumnNumber(AGGREGATE))), r);
            m_aggregated.removeRow(r);
            fire = true;
        }
        if (update && fire) {
    ...
    public Set<AggregateItem> getAggregateSet(Tuple t) {
        int hash = getHashCode(t);
        IntIterator iit = m_aggregated.getIndex(MEMBER_HASH).rows(hash);
        HashSet<AggregateItem> set = new HashSet();
        while (iit.hasNext()) {
            int r = iit.nextInt();
            set.add((AggregateItem) getTuple(m_aggregated.getInt(r, AGGREGATE)));
        }
        return set;
    }

    public boolean containsItem(VisualItem item) {
//        return m_aggregated.getIndex(MEMBER_HASH).rows(getHashCode(item)).hasNext();
        return m_aggregated.getIndex(MEMBER_HASH).get(getHashCode(item)) >= 0;
    }
    ...
    public AggregateItem addItem(Object[] values) {
        if (AGGREGATE_SCHEMA.getColumnCount() != values.length) {
            throw new IllegalArgumentException("Values length must be equal to the aggregate's column count.");
        }
        AggregateItem aggr = (AggregateItem) addItem();
        int aggrRow = aggr.getRow();
        for (int i = 0; i < values.length; i++) {
            String colName = AGGREGATE_SCHEMA.getColumnName(i);
            Class valueType = values[i].getClass();
            if (!aggr.canSet(colName, valueType)) {
                removeRow(aggrRow);
                throw new IllegalArgumentException("Can not assing the value due to different column type : " + valueType);
            }
            aggr.set(colName, values[i]);
        }
        fireTableEvent(aggrRow, aggrRow, EventConstants.ALL_COLUMNS, EventConstants.INSERTED);
        return aggr;
    }
