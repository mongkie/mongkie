...
public class ColumnMetadata implements ColumnListener {
     ...
     * @param table the backing table
     * @param column the name of the column to store metadata for
     * @param cmp a Comparator that determines the default sort order for values
     * in the column
     * @param dynamic indicates if this ColumnMetadata should react to changes
     * in the underlying table values. If true, computed values stored in this
    ...
    /**
     * Update whether the column has multiple values in any single row or not.
     *
     * @param col the column that has changed
     * @param row the column row index that has changed
     * @param prev the previous value at the given location
     * @see Table#columnChanged(prefuse.data.column.Column, int,
     * java.lang.Object)
     * @see Column#MULTI_VAL_SEPARATOR
     */
    public void updateMultipleValueness(Column col, int row, Object prev) {
        if (m_table.getColumn(m_field) != col || col.getColumnType() != String.class) {
            return;
        }
        String val = col.getString(row);
        if (val != null && val.contains(Column.MULTI_VAL_SEPARATOR)) {
            multipleValueRows.add(row);
        } else {
            multipleValueRows.remove(row);
        }
    }

    /**
     * Returns true if the column has multiple values in any single row. Only a
     * column, the type of which is {@link String}, can have multiple values in
     * a single row
     *
     * @return ture if the column has multiple values in any single row, false
     * otherwise.
     */
    public boolean hasMultipleValues() {
        return !multipleValueRows.isEmpty();
    }
    private Set<Integer> multipleValueRows = Collections.synchronizedSet(new HashSet<Integer>());
