...
public class DatabaseDataSource {
    ...
    public synchronized Table getData(PreparedStatement pstmt) throws DataIOException {
        ResultSet rs;
        try {
            long timein = System.currentTimeMillis();
            s_logger.log(Level.INFO, "Issuing query: {0}", pstmt.toString().replaceAll("^.*PreparedStatement@.*: ", ""));
            rs = pstmt.executeQuery();
            long time = System.currentTimeMillis() - timein;
            s_logger.log(Level.INFO, "External query processing completed: {0}.{1} seconds.", new Object[]{time / 1000, time % 1000});
        } catch (SQLException e) {
            throw new DataIOException(e);
        }
        return process(null, rs, null, null);
    }
    ...
    private ResultSet executeQuery(PreparedStatement pstmt) throws SQLException {
        ...
        s_logger.info("Issuing query: " + pstmt);
