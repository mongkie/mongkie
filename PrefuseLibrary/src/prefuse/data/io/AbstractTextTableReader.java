...
public abstract class AbstractTextTableReader extends AbstractTableReader {
    ...
    public Table readTable(InputStream is) throws DataIOException {
        ...
        for (int i = 0; i < ncols; ++i) {
            table.addColumn(getHeaderName(headers, i), di.getType(i));
    ...
    private String getHeaderName(List<String> headers, int idx) {
        String header;
        if (idx < m_headerNames.size()) {
            header = m_headerNames.get(idx);
        } else if (m_hasHeader || idx < headers.size()) {
            header = (String) headers.get(idx);
        } else {
            header = getDefaultHeader(idx);
        }
        return header;
    }
    ...
    public void setHeaderNames(List<String> headerNames) {
        m_headerNames.clear();
        m_headerNames.addAll(headerNames);
    }
    private List<String> m_headerNames = new ArrayList<String>();
