...
public class Schema implements Cloneable, Serializable {
     ...
     */
    public Schema addColumn(String name, Class type) {
    ...
    public Table instantiateBoxingPrimitive() {
        Table t = new Table(0, m_size);
        for (int i = 0; i < m_size; ++i) {
            t.addColumn(m_names[i], boxPrimitive(m_types[i]), m_dflts[i]);
        }
        return t;
    }
